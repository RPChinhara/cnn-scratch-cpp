...
size_t num_neurons = kernel2.shape[0] * 12 * 12;
...
std::array<tensor, 9> forward(const tensor& x, float batch_size) {
    ...
    tensor c1 = relu(c1_z); // 32 + 2P - 5 / S + 1 = 28

    tensor c2_z = convolution(c1, kernel2);
    tensor c2 = relu(c2_z); // 28 + 2P - 5 / S + 1 = 24

    auto [s2, indices_c1_temp] = max_pool(c2);
    indices_c1 = indices_c1_temp;

    // c1: (64, 1, 28, 28) 1 1 1 1 1 1 1 1
    // c2: (64, 1, 24, 24)
    // s2: (64, 1, 12, 12)
...
void train(const tensor& x_train, const tensor& y_train) {
    ...
    for (size_t i = 1; i <= epochs; ++i) {
        ...
        for (size_t j = 0; j < num_batches; ++j) {
            ...
            auto [c1, c1_z, c2_z, s2, f5_z, f5, f6_z, f6, y] = forward(x_batch, batch_size);
            ...
            tensor dl_ds2 = matmul(transpose(w1), dl_df5_z).reshape({static_cast<size_t>(batch_size), kernel2.shape[0], 12, 12});

            tensor dl_dc2 = max_unpool(dl_ds2, indices_c1);
            tensor dl_dc2_z = dl_dc2 * relu_derivative(c2_z);
            tensor dl_dc1 = deconvolution(dl_dc2_z, kernel2);
            ...
            for (size_t i2 = 0; i2 < batch_size; ++i2) {
                tensor s2_sample = slice_4d(c1, i2, 1);
                tensor dl_dc3_sample = slice_4d(dl_dc2_z, i2, 1);
                ...
                for (size_t j2 = 0; j2 < kernel2.shape[0]; ++j2) {
                    tensor dl_dc3_feature_map = slice(dl_dc3_sample, j2 * 24, 24);
                    dl_dc3_feature_map.reshape({1, 1, 24, 24});

                    for (size_t k2 = 0; k2 < kernel2.shape[1]; ++k2) {
                        tensor s2_feature_map = slice(s2_sample, k2 * 28, 28);
                        s2_feature_map.reshape({1, 1, 28, 28});
