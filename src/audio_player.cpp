...
AudioPlayer::AudioPlayer(HWND hwnd) : pDSound(nullptr), pDSoundBuffer(nullptr) {
    if (FAILED(DirectSoundCreate8(nullptr, &pDSound, nullptr))) {
        ...
        MessageBox(NULL, "Failed to create DirectSound.", "Error", MB_ICONERROR);
    }

    if (FAILED(pDSound->SetCooperativeLevel(hwnd, DSSCL_PRIORITY))) {
        // Handle initialization error
        MessageBox(NULL, "Failed to set cooperative level.", "Error", MB_ICONERROR);
    }
}
...
DWORD AudioPlayer::GetAudioFileSize(const char* audioFilePath) {
    HANDLE hFile = CreateFile(audioFilePath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
bool AudioPlayer::LoadAudioData(const char* audioFilePath) {
    ...
    if (FAILED(pDSound->CreateSoundBuffer(&dsbd, &pDSoundBuffer, NULL))) {
        ...
        MessageBox(NULL, "Failed to create sound buffer.", "Error", MB_ICONERROR);
        return false;
    }
    ...
    if (FAILED(pDSoundBuffer->Lock(0, audioDataSize, &pAudioData, &audioDataSizeLocked, NULL, NULL, 0))) {
        // Handle locking error
        MessageBox(NULL, "Failed to lock sound buffer.", "Error", MB_ICONERROR);
        return false;
    }
    ...
    pDSoundBuffer->Unlock(pAudioData, audioDataSizeLocked, NULL, 0);
...
bool AudioPlayer::PlaySound() {
    if (pDSoundBuffer) {
        if (FAILED(pDSoundBuffer->SetCurrentPosition(0))) {
            ...
            MessageBox(NULL, "Failed to set sound buffer position.", "Error", MB_ICONERROR);
            return false;
        }

        if (FAILED(pDSoundBuffer->Play(0, 0, 0))) {
            // Handle playback error
            MessageBox(NULL, "Failed to play sound buffer.", "Error", MB_ICONERROR);
            return false;
        }
