    ...
    for (auto i = 1; i <= epochs; ++i) {
        ...
        for (auto j = seq_length; j > 0; --j) {
            if (j == seq_length) {
                ...
                // d_loss_d_h_t_w_f = matmul(d_loss_d_h_t_w_f * transpose(o_sequence[j] * (1.0f - square(hyperbolic_tangent(c_sequence[j + 1]))) * c_tilde_sequence[j]), vslice(w_i, w_i.shape.back() - 1));
                d_loss_d_h_t_w_i = matmul(d_loss_d_h_t_w_i * transpose(o_sequence[j] * (1.0f - square(hyperbolic_tangent(c_sequence[j + 1]))) * c_tilde_sequence[j]), vslice(w_i, w_i.shape.back() - 1));
