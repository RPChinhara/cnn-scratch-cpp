...
class cnn2d
{
  private:
    ...
    std::vector<ten> forward(const ten &input, const std::vector<ten> &kernel,
                             const size_t stride);

  public:
    cnn2d(const std::vector<size_t> &filters, float const lr);
    void train(const ten &xTrain, const ten &yTrain, const ten &xVal,
               const ten &yVal);
...
class nn
{
  private:
    ...
    std::pair<std::vector<ten>, std::vector<ten>> init_params();
    std::vector<ten> forward(const ten &x, const std::vector<ten> &w,
                             const std::vector<ten> &b);

  public:
    nn(const std::vector<size_t> &lyrs, const std::vector<act_type> &act_types,
       float const lr);
