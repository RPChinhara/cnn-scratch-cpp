...
tensor multihead_cross_attention(const tensor& query, const tensor& key, const tensor& value, const std::vector<std::vector<tensor>>& w, size_t seq_len, size_t d_model, size_t num_heads, bool mask = false);