> LeNet
     ...
     - Check if 2 convs and 2 pooling works, if it does, deconvolution() is working. Thus, whole things are working?
     ...
     - Why is it working now? Is it because model is simply smaller?
          - The inputs had to be transposed before passing to Softmax.
          - Loss was broken.
     - Make changelog.txt to write all of above? Maybe try for some periods?
- AlexNet
